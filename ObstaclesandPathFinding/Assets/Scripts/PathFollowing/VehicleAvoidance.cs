...
public class VehicleAvoidance : MonoBehaviour {
    ...
    public LayerMask obstacleLayerMask; // Set this in the Inspector
    ...
    public void AvoidObstacles(ref Vector3 dir) {
        // Check for obstacles within the avoidance distance
        if (Physics.SphereCast(transform.position, vehicleRadius, transform.forward, out var hit, minimumDistToAvoid, obstacleLayerMask))
        {
            // Adjust direction based on obstacle normal
            Vector3 hitNormal = hit.normal;
            hitNormal.y = 0.0f; // Ignore vertical movement
